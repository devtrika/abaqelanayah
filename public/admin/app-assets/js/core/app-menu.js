...
(function (window, document, $) {
      ...
      $('ul.menu-content').on('click', 'li', function (e) {
        ...
        if ($listItem.is('.disabled')) {
          ...
          if ($listItem.has('ul').length) {
            ...
            // If the li contains a direct <a href>, follow the link immediately
            var $link = $listItem.children('a[href]');
            if ($link.length && $link.attr('href') && $link.attr('href') !== '#' && $link.attr('href') !== 'javascript:void(0);') {
  ...
  // Store open menu state before collapsing
  function storeOpenMenus() {
    var openIndexes = [];
    $('.main-menu li.has-sub').each(function(idx, el) {
      if ($(el).hasClass('open')) {
        openIndexes.push(idx);
      }
    });
    localStorage.setItem('sidebarOpenMenus', JSON.stringify(openIndexes));
  }

  // Restore open menu state after expanding
  function restoreOpenMenus() {
    var openIndexes = JSON.parse(localStorage.getItem('sidebarOpenMenus') || '[]');
    $('.main-menu li.has-sub').each(function(idx, el) {
  ...
  // Hook into sidebar collapse/expand events
