...
use Exception;
use App\Services\Myfatoorah\MyfatoorahApiV2;
...
//-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    private function checkOrderInformation($json, $orderId = null, $price = null, $currncy = null) {
        ...
        $invoiceDisplayValueRaw = (string) ($json->Data->InvoiceDisplayValue ?? '');
        $invoiceDisplayValue = array_values(array_filter(explode(' ', $invoiceDisplayValueRaw), function($v){ return $v !== ''; }));

        if ($price && (!isset($invoiceDisplayValue[0]) || $invoiceDisplayValue[0] != $price)) {
            return false;
        }
        if ($currncy && (!isset($invoiceDisplayValue[1]) || $invoiceDisplayValue[1] != $currncy)) {
            return false;
        }
