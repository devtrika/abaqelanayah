...
class CancelRequestOrderController extends Controller
{
    ...
    public function show($id)
    {
        ...
        ])->where('status', 'request_cancel')->findOrFail($id);
    ...
    public function acceptCancelRequest(Request $request, $id)
    {
        ...
        try {
            return DB::transaction(function () use ($request, $id) {
                ...
                return response()->json([
                    'success' => true,
                  'message' => __('admin.cancel_request_accepted_successfully'),
                    'data' => [
                        'refund_amount' => $refundAmount,
                        'cancel_fees' => $cancelFees
                    ],
                    'redirect_url' => url('/admin/orders/' . $order->id . '/show'),
                ]);
            });
    ...
    public function rejectCancelRequest(Request $request, $id)
    {
        ...
        try {
            return DB::transaction(function () use ($request, $id) {
                ...
                // Update order status to indicate cancel request was rejected
                $order->update([
                    'status' => OrderStatusEnum::REQUEST_REJECTED->value,
                ]);
                ...
                return response()->json([
                    'success' => true,
                    'message' => __('admin.cancel_request_rejected_successfully'),
                    'redirect_url' => url('/admin/orders/' . $order->id . '/show'),
