    ...
    <div class="invoice-container">
                    ...
                    @foreach($providers as $provider)
                        <div class="info-item" style="border-bottom: 1px solid #eee; padding-bottom: 10px; margin-bottom: 10px;">
                            ...
                            <div class="info-item">
                                ...
                                <span class="info-value">{{ $provider['sub_order_number'] }}</span>
                            </div>
                            <div class="info-item">
                                <span class="info-label">رقم الهاتف:</span>
                            ...
                            <div class="info-item">
                                ...
                                <span class="status-badge status-{{ $provider['status'] }}">
                                    {{ __('admin.' . $provider['status']) }}
                                </span>
                            </div>
                            <div class="info-item">
                                <span class="info-label">إجمالي الطلب:</span>
                                <span class="info-value">{{ number_format($provider['total'], 2) }} ريال</span>
                            </div>
                        </div>
                    @endforeach
                @else
        ...
        <div class="info-section">
            <div class="info-box">
                ...
                <div class="info-item">
                    <span class="info-label">حالة الطلب:</span>
                    @php $effectiveStatus = $order->refund_status ?? $order->status ?? $order->current_status; @endphp
                ...
                <div class="info-item">
                    ...
                    <span class="info-value">{{ __('admin.' . $order->payment_status) }}</span>
                </div>
            </div>
            ...
            <tbody>
                @foreach($items as $item)
                    <tr>
                        ...
                        <td>
                            @if($item->item && $item->item->provider)
                                {{ $item->item->provider->commercial_name ?? $item->item->provider->user->name }}
                            @else
                                غير محدد
                            @endif
                        </td>
