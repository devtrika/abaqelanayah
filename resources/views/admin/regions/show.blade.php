...
<section id="multiple-column-form">
    ...
    {{-- Cities List for this Region --}}
    <div class="row mt-4">
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">{{ __('admin.cities') }}</h4>
                </div>
                <div class="card-body">
                    <table class="table">
                        <thead>
                        ...
                        @forelse($cities as $city)
                            <tr>
                                <td>
                                    @foreach(languages() as $lang)
                                        <span class="badge badge-light">{{ $city->getTranslations('name')[$lang] ?? '' }}</span>
                                    @endforeach
                                </td>
                                <td>{{ \Carbon\Carbon::parse($city->created_at)->format('d/m/Y') }}</td>
                                <td>
                                    <a href="{{ route('admin.cities.show', ['id' => $city->id]) }}" class="btn btn-warning btn-sm"><i class="feather icon-eye"></i> {{ __('admin.show') }}</a>
                                    <a href="{{ route('admin.cities.edit', ['id' => $city->id]) }}" class="btn btn-primary btn-sm"><i class="feather icon-edit"></i>{{ __('admin.edit') }}</a>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="3">{{ __('admin.there_are_no_matches_matching') }}</td></tr>
                        @endforelse
                        </tbody>
                    </table>
                </div>
