...
<div class="card shadow-sm border-0 mb-4">
 ...
 @if($order->notes || $order->problem)
    <div class="card mb-4">
        <div class="card-header bg-danger text-white">
            <h5 class="mb-0">
                <i class="feather icon-alert-triangle mr-2"></i>
                {{ __('admin.problem_details') }}
            </h5>
        </div>
        <div class="card-body">
            {{-- <div class="mb-2">
                <strong>{{ __('admin.problem_purpose') }}:</strong>
                <span>{{ $order->problem->purpose ?? '-' }}</span>
            </div> --}}
            <div>
                <strong>{{ __('admin.problem_reason') }}:</strong>
                @php
                    if (!empty($order->notes)) {
                        $problemReasonText = $order->notes;
                    } else {
                        $problemReasonText = $order->problem->problem ?? null;
                ...
                @endphp
                <span>{{ $problemReasonText }}</span>
            </div>
            <div class="mt-3">
                @php
                    // Extract reporter info from order status history (map_desc)
                    $problemStatus = null;
                    ...
                    <div class="timeline-body">
                        @if($statusChange->map_desc)
                            @php
                                ...
                                $map = json_decode($desc, true);
                                $hasReporter = false;
                                $rType = null; $rName = null; $rPhone = null;
                                if (is_array($map)) {
                                    if (isset($map['reporter']) && is_array($map['reporter'])) {
                                        $rep = $map['reporter'];
                                        $rType = $rep['type'] ?? null;
                                        $rName = $rep['name'] ?? null;
                                        $rPhone = $rep['phone'] ?? null;
                                        $hasReporter = true;
                                    } else {
                                        // legacy shape: { reported_by: 'delivery'|'client', name: '', phone: '' }
                                        $reportedBy = $map['reported_by'] ?? ($map['reportedBy'] ?? null);
                                        $name = $map['name'] ?? $map['reporter_name'] ?? null;
                                        $phone = $map['phone'] ?? $map['reporter_phone'] ?? null;
                                        if ($reportedBy || $name || $phone) {
                                ...
                                if ($hasReporter) {
                                    if (empty($rName)) {
                                        $rName = $rType === 'delivery' ? ($order->deliveryUser?->name ?? '-') : ($order->user?->name ?? '-');
                                    }
                                    if (empty($rPhone)) {
                                        $rPhone = $rType === 'delivery' ? ($order->deliveryUser?->phone ?? '-') : ($order->user?->phone ?? '-');
                                    }
                                    $typeLabel = $rType === 'delivery' ? 'مندوب التوصيل' : ($rType === 'client' ? 'العميل' : $rType ?? '-');
                                }
                            @endphp
