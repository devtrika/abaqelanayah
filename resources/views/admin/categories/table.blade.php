        ...
        <tbody>
            @foreach($categories as $category)
                <tr class="delete_row">
                    ...
                    <td>{{ $category->id }}</td>
                    <td><img src="{{$category->image_url}}" width="30px" height="30px" alt=""></td>
                    <td>{{ $category->getTranslations('name')[app()->getLocale()] ?? $category->getTranslations('name')[array_key_first($category->getTranslations('name'))] }}</td>

                    <td>
                        @if($category->parent_id)
                            {{ optional($category->parent)->getTranslations('name')[app()->getLocale()] ?? optional($category->parent)->getTranslations('name')[array_key_first(optional($category->parent)->getTranslations('name'))] }}
                        @else
                            -
                        @endif
                    </td>
