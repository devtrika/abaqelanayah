...
@section('title', __('site.myAccount') . ' - ' . __('site.edit_address'))

@section('meta_description', __('site.edit_address'))
    ...
    <section class="breadcrumb-section">
      <div class="container">
        <ul class="breadcrumb-list">
          ...
          <li class="breadcrumb-item">
            <span class="breadcrumb-link"> {{ __('site.edit_address') }} </span>
          </li>
        </ul>
      </div>
    </section>
        ...
        <div class="account-content">
          ...
          <div class="account-main">
            <div class="account-header">
              <h2 class="account-title">{{ __('site.edit_address') }}</h2>
            </div>
            <form id="addressForm" method="POST" action="{{ route('website.addresses.update', $address) }}" data-validate>
              @csrf
              @method('PUT')
              <div class="form-grid grid-3">
                <div class="form-group">
                  <label for="name" class="form-label">{{ __('site.address_name') }}</label>
                  <input type="text" class="form-control" name="address_name" value="{{ old('address_name', $address->address_name) }}" data-rules="required|minLength:2|maxLength:100" />
                </div>
                <div class="form-group">
                  <label for="name" class="form-label">{{ __('site.recipient_name') }}</label>
                  <input type="text" class="form-control" name="name" value="{{ old('name', $address->recipient_name) }}" data-rules="required|minLength:2|maxLength:100" />
                </div>
                <div class="form-group">
                  ...
                  <select class="form-control" name="city" select2 data-rules="required">
                    ...
                    @isset($cities)
                      @foreach($cities as $city)
                        <option value="{{ $city->id }}" {{ (old('city', $address->city_id) == $city->id) ? 'selected' : '' }}>{{ $city->name }}</option>
                      @endforeach
                    @endisset
                  </select>
                </div>
                  ...
                  <select class="form-control" name="district" select2 data-rules="required">
                    ...
                    @isset($districts)
                      @foreach($districts as $district)
                        <option value="{{ $district->id }}" {{ (old('district', $address->districts_id) == $district->id) ? 'selected' : '' }}>{{ $district->name }}</option>
                      @endforeach
                    @endisset
                  </select>
                </div>
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="address_desc" value="{{ old('address_desc', $address->description) }}" data-rules="maxLength:500" />
                </div>
                <div class="form-group">
                  ...
                  <div class="map-input">
                    ...
                    <input
                      ...
                      value="{{ old('address_map', ($address->latitude && $address->longitude) ? $address->latitude.', '.$address->longitude : '') }}"
                      readonly
                    />
...
<script>
  (function($){
    function populateDistricts(cityId, selectedId){
      ...
      $.get("{{ url('/api/city') }}/" + cityId + "/districts")
        .done(function(res){
          ...
          $district.empty();
          $district.append('<option value="" hidden>{{ __('site.choose_from_list') }}</option>');
          list.forEach(function(d){
            var sel = (String(selectedId) === String(d.id)) ? ' selected' : '';
            $district.append('<option value="'+ d.id +'"'+ sel +'>'+ (d.name || d.title || d.label) +'</option>');
          });
        })
    ...
    $('select[name="city"]').on('change', function(){
      var cid = $(this).val();
      if (cid){ populateDistricts(cid, null); }
    });

    // In case city changes via old input or initial, ensure districts are synced
    var initialCity = $('select[name="city"]').val();
    var initialDistrict = '{{ old('district', $address->districts_id) }}';
    if (initialCity){ populateDistricts(initialCity, initialDistrict); }
    ...
    function openMapPicker($display, $lat, $lng){
      ...
      if(!modalEl){ if(window.toastr) toastr.error("{{ __('site.map_picker_unavailable') }}"); return; }
      ...
      setTimeout(function(){
        ...
        if(map){
          map.invalidateSize();
          // If editing and has existing coordinates, show them on map
