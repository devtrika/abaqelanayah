<div class="content-body">
        ...
        <div class="row m-0 justify-content-between">
            ...
            @isset($addbutton)
                @php
                    ...
                    $addRouteName = null;
                    try {
                        $addRouteName = app('router')->getRoutes()->match(
                            app('request')->create($addbutton)
                        )->getName();
                    } catch (\Exception $e) {
                        // If route matching fails, check if it's a named route
                        $addRouteName = $addbutton;
                    }
                @endphp
                @if(adminCan($addRouteName))
            ...
            @isset($deletebutton)
                @php
                    ...
                    try {
                        $deleteRouteName = app('router')->getRoutes()->match(
                            app('request')->create($deletebutton)
                        )->getName();
                    } catch (\Exception $e) {
                        $deleteRouteName = $deletebutton;
                    }
                @endphp
                @if(adminCan($deleteRouteName))
    ...
    <!-- Add initial loading state to empty table_content_append divs -->
    <script>
        $(document).ready(function() {
            // Check if table_content_append is empty and add initial loading
            if ($('.table_content_append').length > 0 && $('.table_content_append').is(':empty')) {
